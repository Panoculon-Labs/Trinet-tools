...
| 32     | 4    | uint32   | readout_time_us   | v4: rolling-shutter readout span µs (valid when `entry_flags` bit 2); per-row delay = `readout_time_us / image_height` (Kalibr `line_delay`). `0` with bit 2 set = global shutter |
